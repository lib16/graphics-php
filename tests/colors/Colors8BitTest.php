...
class Colors8BitTest extends TestCase
{
    /**
    ...
    public function testColor(
        ...
        $this->assertEquals(
            $expected,
            Colors8Bit::color($hue, $saturation, $white)
        );
    }
     ...
     */
    public function testFromRgb(int $expected, int $rgb, float $opacity = null)
    {
        $this->assertEquals($expected, Colors8Bit::fromRgb($rgb, $opacity));
    }

    public function fromRgbProvider(): array
    {
        return [
            [Colors8Bit::argb(0, 127, 255), 0x007fff],
            [Colors8Bit::argb(0, 127, 255, 0), 0x007fff, 0],
            [Colors8Bit::argb(0, 127, 255, 128), 0x007fff, 0.5],
            [Colors8Bit::argb(0, 127, 255, 255), 0x007fff, 1]
        ];
    }
    ...
    {
        $this->assertEquals(
            Colors8Bit::argb(120, 120, 120),
            Colors8Bit::gray(Colors8Bit::argb(120, 120, 120))
        );
        $this->assertEquals(
            40,
            Colors8Bit::min(Colors8Bit::argb(120, 80, 40))
        );
        $this->assertEquals(
            120,
            Colors8Bit::max(Colors8Bit::argb(120, 80, 40))
        );
    }
}
