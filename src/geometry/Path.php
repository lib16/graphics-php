...
class Path extends PathCommands
{
	...
	public function a(float $rx, float $ry,
			Angle $xAxisRotation = null, bool $largeArc, bool $sweep, Point $point): self
	{
		$this->commands[] = new Arc($rx, $ry, $xAxisRotation, $largeArc, $sweep, $point);
		return $this;
	}
	...
	public function ellipse(Point $center, float $rx, float $ry, Angle $xAxisRotation = null): self
	{
	...
	public function sector(Point $center, Angle $start, Angle $stop, float $radius): self
	{
		$points = Points::sector($center, $start, $stop, $radius);
		$this->reorderPoints($points, 1);
		$this->m($points[0]);
		$this->l($points[1]);
		$this->a($radius, $radius, null, $this->largeArc($start, $stop), !$this->ccw, $points[2]);
		$this->z();
		return $this;
	}

	public function ringSector(Point $center,
			Angle $start, Angle $stop, float $radius, float $innerRadius): self
	{
		$largeArc = $this->largeArc($start, $stop);
		if ($this->ccw) {
			$swap = $start; $start = $stop; $stop = $swap;
		}
		$points = Points::ringSector($center, $start, $stop, $radius, $innerRadius);
		$this->m($points[0]);
		$this->a($radius, $radius, null, $largeArc, !$this->ccw, $points[1]);
		$this->l($points[2]);
		$this->a($innerRadius, $innerRadius, null, $largeArc, $this->ccw, $points[3]);
		$this->z();
		return $this;
	}
